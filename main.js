...
const { color } = require('./LorenzoLibrary/color')
const FileType = require('file-type')
const path = require('path')
const axios = require('axios')
const _ = require('lodash')
const { uncache, nocache } = require('./LorenzoLibrary/loader')
const yargs = require('yargs/yargs')
const { Low, JSONFile } = require('./LorenzoLibrary/lowdb')
const moment = require('moment-timezone')
const PhoneNumber = require('awesome-phonenumber')
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./LorenzoLibrary/exif')
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetch, await, sleep, reSize } = require('./LorenzoLibrary/myfunc')
...
global.db = new Low(new JSONFile(`./LorenzoSource/database.json`))
...
require('./LorenzoCheems.js')
nocache('../LorenzoCheems.js', module => console.log(color('[ CHANGE ]', 'green'), color(`'${module}'`, 'green'), 'Updated'))
require('./main.js')
nocache('../main.js', module => console.log(color('[ CHANGE ]', 'green'), color(`'${module}'`, 'green'), 'Updated'))

let phoneNumber = "628123456789"
let owner = JSON.parse(fs.readFileSync('./LorenzoSource/data/role/owner.json'))
...
async function sendEmail(to, subject, htmlContent) {
    ...
    let mailOptions = {
        from: `"LorenzoBotInc" <${global.pairing.email}>`,
        to: to,
        subject: subject,
        html: htmlContent
    };
    ...
    const LorenzoBotInc = makeWASocket({
     ...
     auth: {
      ...
      getMessage: async (key) => {
            ...
            return {
                conversation: "LorenzoCheems Bot Here!"
            }
        },
   ...
   if (pairingCode && !LorenzoBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +628123456789")))
            process.exit(0)
         }
      } else {
...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
	        ...
	        console.log(color(`\n${themeemoji} YT CHANNEL: Lorenzxz`,'magenta'))
            console.log(color(`${themeemoji} GITHUB: Lorenzxz `,'magenta'))
            console.log(color(`${themeemoji} INSTAGRAM: @LLorenzxz `,'magenta'))
...
ppgroup = 'https://i.ibb.co/RBx5SQC/avatar-group-large-v2.png?q=60'
    ...
    LorenzoBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./LorenzoCheems")(LorenzoBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    LorenzoBotInc.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
     ...
     return LorenzoBotInc.sendMessage(jid, { audio: await getBuffer(url), caption: caption, mimetype: 'audio/mpeg', ...options}, { quoted: quoted, ...options })
      ...
      LorenzoBotInc.getFile = async (PATH, save) => {
        ...
        filename = path.join(__filename, '../LorenzoSource/' + new Date * 1 + '.' + type.ext)
...
LorenzoBotInc.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
       ...
       if (options.asSticker || /webp/.test(mime)) {
        let { writeExif } = require('./LorenzoLibrary/exif')
