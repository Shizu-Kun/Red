...
/*!-======[ Module exports ]======-!*/
module.exports = Lorenzo = async (Lorenzo, bot) => {
    //console.log(Lorenzo)
    try {
        const body = Lorenzo.message.text || Lorenzo.message.caption || ''
        const budy = (typeof Lorenzo.message.text == 'string' ? Lorenzo.message.text : '')
        const {
            isUrl
        } = simple
        ...
        const user = simple.getUserName(Lorenzo.message.from)
        const pushname = user.full_name;
        const user_id = Lorenzo.message.from.id + " "
        const username = Lorenzo.message.from.username ? Lorenzo.message.from.username : "atzmus";
        const isCreator = OWNER[0].replace("https://t.me/", '') == Lorenzo.update.message.from.username
        const from = Lorenzo.message.chat.id

        const isGroup = Lorenzo.chat.type.includes('group')
        const groupName = isGroup ? Lorenzo.chat.title : ''

        const isImage = Lorenzo.message.hasOwnProperty('photo')
        const isVideo = Lorenzo.message.hasOwnProperty('video')
        const isAudio = Lorenzo.message.hasOwnProperty('audio')
        const isSticker = Lorenzo.message.hasOwnProperty('sticker')
        const isContact = Lorenzo.message.hasOwnProperty('contact')
        const isLocation = Lorenzo.message.hasOwnProperty('location')
        const isDocument = Lorenzo.message.hasOwnProperty('document')
        const isAnimation = Lorenzo.message.hasOwnProperty('animation')
        const isMedia = isImage || isVideo || isAudio || isSticker || isContact || isLocation || isDocument || isAnimation
        const quotedMessage = Lorenzo.message.reply_to_message || {}
        ...
        const isQuoted = Lorenzo.message.hasOwnProperty('reply_to_message')
        const timestampi = speed();
        const latensii = speed() - timestampi

        const reply = async (text) => {
            for (var x of simple.range(0, text.length, 4096)) { //maks 4096 character, jika lebih akan eror
                return await Lorenzo.replyWithMarkdown(text.substr(x, 4096), {
...
/*!-======[ Case ]======-!*/       
        switch (command) {
            ...
            case 'owner':
            case 'creator': {
                await Lorenzo.sendContact(OWNER_NUMBER, OWNER_NAME)
            ...
            case 'script':
            case 'scrip': {
                Lorenzo.reply("Source code this bot", {
                    reply_markup: {
                        inline_keyboard: [
                            [{
                                text: 'Github',
                                ...
                                text: 'YouTube',
                                url: "https://www.youtube.com/@Lorenzo"
                            }]
                        ]
                    }
                })
            ...
            case 'help': {
                ...
                lang.menu(Lorenzo, THUMBNAIL, pushname, OWNER_NAME, OWNER, prefix, hitall, latensii, os, simple, week, date, dateIslamic, username, isCreator, Lorenzo.message.from.id.toString())
            }
            break
            //anime
            case "akame":
...
break
    ...
    break
            ...
            break
            case 'play': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} judul lagu\`\n\nContoh penggunaan:\n\`${prefix+command} bot WhatsApp Lorenzo\``)
...
📝 Description : ${dapet[v].description}\n\n---------------------------\n\n`
            ...
            case "mf":
            case "mediafire": {
                if (!args[0]) return reply(`Kirim perintah:\n\`${prefix+command} link mediafire\`\n\nContoh penggunaan:\n\`${prefix+command} https://www.mediafire.com/file/eb14v8x4oz7ok3h/Lorenzobot-Mdv17.5-withModule.zip/file\``)
            ...
            case "tiktoknowm":
            case "tiktok": {
                if (!args[0]) return reply(`Kirim perintah:\n\`${prefix+command} link tiktok video\`\n\nContoh penggunaan:\n\`${prefix+command} https://www.tiktok.com/@Lorenzo.official/video/7210229439744003355?_r=1&u_code=e44201c8bfkd30®ion=ID&mid=7202111782981913370&preview_pb=0&language=id&_d=e0cah74j08m7c7&share_item_id=7210229439744003355&source=h5_t×tamp=1679216331&user_id=7148061777321133083&sec_user_id=MS4wLjABAAAA50SieLfP2YD-R-gqSE3svcPxaPqr_53pA6RKyJUkQo_AreOGrLDiVRnajBVglVIk&utm_source=copy&utm_campaign=client_share&utm_medium=android&share_iid=7160625938232592154&share_link_id=6b2fea89-b038-4919-8d58-38b4efba5b9c&share_app_id=1180&ugbiz_name=Main&ug_btm=b8727%2Cb2878\``)
            ...
            /*   case "apple":
               ...
               case "google": {
                   ...
                   let savestik = await Lorenzo.sendImageAsSticker(m.chat, img, m, {
                       packname: packname,
                       author: author
                   })
            ...
            break
            case 'colorfulpubg': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|color\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|green-yellow\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|color\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|green-yellow\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'astronotspace': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|style\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|panther\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|style\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|panther\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'wallpaperaov': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|heroes\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|preyta-2\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|heroes\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|preyta-2\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            case 'effect3donbeach': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|background\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|beach-5\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|background\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|beach-5\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'cutegirlgamer': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|logo\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|style-3\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|logo\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|style-3\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'pubgcutelogo': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|logo\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|chicken\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|logo\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|chicken\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            case 'logogamingassasin': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|logo\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|style-1\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|logo\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|style-1\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'gaminglogo4fvs': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|logo\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|soldier\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|logo\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|soldier\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'lolpentakill': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|style\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|nocturne\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|style\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|nocturne\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            case 'avatarleagueofking': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|style\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|ilumia-5\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|style\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|ilumia-5\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'rovwallpaper': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|avatar\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|grakk-6\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|avatar\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|grakk-6\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            case 'beautifulgalaxylol': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|style\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|khazix-2\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|style\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|khazix-2\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'crossfirecover': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|character\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|natahari\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|character\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|natahari\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'avatar3q360': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|avatar\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|truong_phi\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|avatar\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|truong_phi\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'coverofwarface': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|character\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|warface\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|character\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|warface\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'newlolavatar': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|avatar\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|shyvana\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|avatar\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|shyvana\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'overwatchcover': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|hero\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|zsymmetra\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|hero\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|zsymmetra\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'crossfirestyle': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|avatar\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|thefates\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|avatar\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|thefates\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'lolcoverbyname': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|avatar\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|xinzhao\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|avatar\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|xinzhao\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'bannerytcsgo': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|template\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|banner-7\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|template\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|banner-7\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            case 'fbgamepubgcover': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks|template\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|cover-4\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks|template\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|cover-4\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            case 'bannerofoverwatch': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks1|teks2|background\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|reinhardt\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks1|teks2|background\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|reinhardt\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'mascotstyle': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks1|teks2|thumb\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|kitsune\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks1|teks2|thumb\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|kitsune\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            case 'avataroverwatch': {
                if (!text) return reply(`Kirim perintah:\n\`${prefix+command} teks1|teks2|thumb\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|zenyatta\``)
                if (!text.includes('|')) return reply(`Kirim perintah:\n\`${prefix+command} teks1|teks2|thumb\`\n\nContoh penggunaan:\n\`${prefix+command} Lorenzo|zenyatta\``)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            /*  case "patrick":
              ...
              case "tyni": {
                  ...
                  let res = await fetch(ini_url)
                  if (!res.ok) throw await res.text()
                  Lorenzo.replyWithSticker({url: ini_url})
              }
              break
              //canvas
              case 'ttp':
              case 'attp': {
                  ...
                  if (!res.ok) throw await res.text()
                  let img = await res.buffer()
                  let encmedia = await Lorenzo.sendMediaAsSticker(m.chat, img, m, {
...
Contoh:
\`${prefix}nuliskiri Subscribe Ya YT Lorenzo\``)
                break
            case 'foliokiri':
            case 'foliokanan':
            case 'nuliskanan':
