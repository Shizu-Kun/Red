...
async function startLorenzo() {
    bot.on('callback_query', async (Lorenzo) => {
        //console.log(Lorenzo)
        action = Lorenzo.callbackQuery.data.split(' ')
        args = action
        user_id = Number(action[1])
        if (Lorenzo.callbackQuery.from.id != user_id) return Lorenzo.answerCbQuery('Uppss... this button not for you!', {
            show_alert: true
        })
        const timestampi = speed();
        const latensii = speed() - timestampi
        const user = simple.getUserName(Lorenzo.callbackQuery.from)
        const {
            isUrl,
            fetchJson
        } = simple
        const pushname = user.full_name;
        const username = user.username ? user.username : "lorenzobotinc";
        const isCreator = [Lorenzo.botInfo.username, ...global.OWNER].map(v => v.replace("https://t.me/", '')).includes(user.username ? user.username : "-")
...
/*!-======[ Case ]======-!*/        
        try {
            switch (action[0]) {
                case "menucmd": {
                    let hit_total;
                    try {
                        hit_total = await simple.fetchJson('https://api.countapi.xyz/hit/api-Lorenzobot.herokuapp.com/visits')
                    } catch {
                        hit_total = {
                            value: "-"
                        }
                    ...
                    lang.menu(Lorenzo, THUMBNAIL, pushname, OWNER_NAME, OWNER, "/", hitall, latensii, os, simple, week, date, dateIslamic, username, isCreator, user.id.toString())
                }
                break
                case "animecmd": {
                    lang.animecmd(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "asupancmd": {
                    lang.asupancmd(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "cecancmd": {
                    lang.cecancmd(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "cogancmd": {
                    lang.cogancmd(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "downloadcmd": {
                    lang.downloadcmd(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "ephotocmd": {
                    lang.ephotocmd(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "ephotocmd2": {
                    lang.ephotocmd2(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "logocmd": {
                    lang.logocmd(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "logocmd2": {
                    lang.logocmd2(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "islamcmd": {
                    lang.islamcmd(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "nsfwcmd": {
                    lang.nsfwcmd(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "photooxycmd": {
                    lang.photooxycmd(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "textprocmd": {
                    lang.textprocmd(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "textprocmd2": {
                    lang.textprocmd2(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "textprocmd3": {
                    lang.textprocmd3(Lorenzo, THUMBNAIL, user_id.toString())
                }
                break
                case "owner": {
    ...
    bot.command('help', async (Lorenzo) => {
        user = simple.getUserName(Lorenzo.message.from)
        await Lorenzo.reply(lang.first_chat(BOT_NAME, user.full_name), {
            parse_mode: "MARKDOWN",
            disable_web_page_preview: true,
            reply_markup: {
                inline_keyboard: [
    ...
    bot.command('start', async (Lorenzo) => {
        let user = simple.getUserName(Lorenzo.message.from)
        await Lorenzo.reply(lang.first_chat(BOT_NAME, user.full_name), {
            parse_mode: "MARKDOWN",
            disable_web_page_preview: true,
            reply_markup: {
...
GoodBye~~`
    ...
    bot.on('message', async (Lorenzo) => {
        require("../LorenzoTelegram/LorenzoMain")(Lorenzo, bot)
    })
    ...
    bot.telegram.getMe().then((getme) => {
        console.table({
            ...
            "Author": "https://t.me/lorenzobotinc"
        })
    })
}
startLorenzo()
